...
from samesh.models.sam import combine_bmasks, colormap_mask
...
class SegmentationModelMesh(nn.Module):
    ...
    def render(self, scene: Scene, visualize_path=None, view_name=None) -> dict[str, NumpyTensor]:
        ...
        renders = {
            ...
            'norms': [output['norms']]  # 添加法线信息到渲染结果
        }
    ...
    def call_segmentation(self, image: Image, mask: NumpyTensor['h w'], view_index=None):
        ...
        assets_dir = os.path.join(project_root, 'assert')
...
def segment_mesh(filename: Path | str, config: OmegaConf, predict_image_path: str = None, visualize=False, extension='glb', target_labels=None, texture=False, view_name=None) -> Trimesh:
    ...
    try:
        ...
        if view_name and view_name in VIEW_POSITIONS:
            camera_info = VIEW_POSITIONS[view_name]
            print(f"相机位置文件未找到，使用预定义的'{view_name}'视角: {camera_info['position']}")
        else:
            # 默认使用center视角
            camera_info = VIEW_POSITIONS['center']
            print(f"相机位置文件未找到，使用默认的'center'视角: {camera_info['position']}")
    ...
    if hasattr(model, 'set_camera_position'):
        ...
        up = camera_info['up']
        print(f"已设置相机位置:")
        print(f"  - 位置坐标: ({pos[0]:.4f}, {pos[1]:.4f}, {pos[2]:.4f})")
        print(f"  - 目标点: ({target[0]:.4f}, {target[1]:.4f}, {target[2]:.4f})")
        print(f"  - 上方向: ({up[0]:.4f}, {up[1]:.4f}, {up[2]:.4f})")
...
if __name__ == '__main__':
    ...
    if args.model:
        ...
        # 处理所有模型
        pattern = os.path.join(assets_dir, '*.obj')
    ...
    # 打印所有可用视角的位置信息
    print("\n可用的所有视角位置信息:")
    for view_key, view_data in VIEW_POSITIONS.items():
        pos = view_data["position"]
        print(f"  - {view_key}: position=({pos[0]:.2f}, {pos[1]:.2f}, {pos[2]:.2f})")
    print()
    
    # 打印使用的视角信息
    if args.view:
        view_pos = VIEW_POSITIONS[args.view]["position"]
        view_target = VIEW_POSITIONS[args.view]["target"]
        view_up = VIEW_POSITIONS[args.view]["up"]
        print(f"使用单一视角模式: '{args.view}'")
        print(f"  位置坐标: ({view_pos[0]:.2f}, {view_pos[1]:.2f}, {view_pos[2]:.2f})")
        print(f"  目标点: ({view_target[0]:.2f}, {view_target[1]:.2f}, {view_target[2]:.2f})")
        print(f"  上方向: ({view_up[0]:.2f}, {view_up[1]:.2f}, {view_up[2]:.2f})")
