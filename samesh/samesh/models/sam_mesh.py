...
#                               固定路径配置
# ------------------------------------------------------------------------------
PREDICT_DIR = "/hy-tmp/PartField_Sketch/segmentation/data/predict"
URDF_DIR    = "/hy-tmp/PartField_Sketch/segmentation/data/urdf"
RESULT_DIR  = "/hy-tmp/PartField_Sketch/segmentation/data/samesh_result"
FAILURE_DIR = os.path.join(RESULT_DIR, "failure")   # 新增：集中放失败 PNG
CONFIG_FILE = Path(__file__).resolve().parents[2] / "configs" / "mesh_segmentation.yaml"
...
_PREDICT_RE = re.compile(
    r"""^(?P<class>.+?)_(?P<id>\d+)_segmentation_(?P<view>.+?)_joint_(?P<joint>\d+)_predict\.png$""",
    re.IGNORECASE
)
...
def segment_mesh(
    ...
    # 不在这里创建目录，而是在调用成功后由外部创建
    # os.makedirs(config.output, exist_ok=True)  # 移除这行
...
def _process_all():
    ...
    pngs = sorted(glob.glob(os.path.join(PREDICT_DIR, "*_predict.png")))
    ...
    for idx, png in enumerate(pngs, 1):
        ...
        mesh_path = Path(URDF_DIR) / mid / "yy_object" / f"{mid}.obj"
