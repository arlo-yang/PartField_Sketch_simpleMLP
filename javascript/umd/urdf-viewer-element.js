(function (global, factory) {
    ...
    class URDFViewer extends HTMLElement {
        ...
        constructor() {
            ...
            // 额外的方向光，用于从不同角度提供照明，减少阴影区域
            const secondaryDirLight = new THREE__namespace.DirectionalLight(0xffffff, 0.7 * Math.PI);
            secondaryDirLight.position.set(-4, 8, -2);
            secondaryDirLight.castShadow = false; // 不投射阴影，仅用于照亮模型
            scene.add(secondaryDirLight);
            scene.add(secondaryDirLight.target);
            
            // 额外的环境光，用于提供均匀的基本照明
            const ambientFillLight = new THREE__namespace.AmbientLight(0xffffff, 0.3);
            ...
            this.secondaryDirLight = secondaryDirLight;
            this.ambientLight = ambientLight;
            this.ambientFillLight = ambientFillLight;
        ...
        attributeChangedCallback(attr, oldval, newval) {
            ...
            switch (attr) {
                ...
                case 'ambient-color': {
                    ...
                    // 同时更新环境填充光的颜色，保持其淡化版本
                    if (this.ambientFillLight) {
                        // 创建颜色对象进行处理
                        const fillColor = new THREE__namespace.Color(this.ambientColor);
                        // 保持原有色调，但亮度提高，饱和度降低
                        fillColor.offsetHSL(0, -0.1, 0.1);
                        this.ambientFillLight.color.copy(fillColor);
                    }
        ...
        _updateEnvironment() {
            ...
            // 同时更新第二个方向光的位置，使其也始终对准模型中心
            if (this.secondaryDirLight) {
                // 保存初始相对偏移
                const secondaryOffset = this.secondaryDirLight.position.clone().sub(this.secondaryDirLight.target.position);
                
                // 设置目标为模型中心，并相应地调整光源位置
                this.secondaryDirLight.target.position.copy(center);
                this.secondaryDirLight.position.copy(center).add(secondaryOffset);
            }
