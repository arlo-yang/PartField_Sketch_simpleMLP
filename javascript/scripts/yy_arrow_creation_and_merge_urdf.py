...
def compute_node_objs_volume_any(node, base_path):
    """计算该节点所有 'objs' 合并后 bounding box 的体积，若无可用 objs 返回 0"""
    all_meshes = []
    if "objs" in node:
        for obj_name in node["objs"]:
            mesh_path = os.path.join(base_path, "textured_objs", f"{obj_name}.obj")
            if os.path.exists(mesh_path):
                try:
                    mesh = trimesh.load_mesh(mesh_path)
                    all_meshes.append(mesh)
                except Exception as e:
                    print(f"[GlobalVolume] 加载 {obj_name}.obj 出错: {e}")
    if not all_meshes:
        return 0.0
    merged = trimesh.util.concatenate(all_meshes)
    return np.prod(merged.bounding_box.extents)


def compute_node_objs_volume_slider(node, base_path):
    """类似 compute_node_objs_volume_any，只是演示另写法"""
    arr=[]
    if "objs" in node:
        for objn in node["objs"]:
            mp = os.path.join(base_path,"textured_objs",f"{objn}.obj")
            if os.path.exists(mp):
                try:
                    mm = trimesh.load_mesh(mp)
                    arr.append(mm)
                except Exception as e:
                    print("[sliderVolume] load fail:", e)
    if not arr:
        return 0.0
    merged = trimesh.util.concatenate(arr)
    return np.prod(merged.bounding_box.extents)


def enrich_node(node, mobility_dict, base_path):
    """
    遍历 node(DFS)，若 name在mobility_dict => 给 joint/jointData
    hinge > slider (只在本节点生效)
    """
    name = node.get("name")
    if name and name in mobility_dict:
        jinfo = mobility_dict[name]
        jt = jinfo.get("joint")
        jd = jinfo.get("jointData", {})
        no_limit = jd.get("limit",{}).get("noLimit",False)
        if not no_limit:
            def motion_score(jd_):
                lim= jd_.get("limit",{})
                return abs(lim.get("b",0)-lim.get("a",0))
            ms = motion_score(jd)
            if jt=="hinge":
                if node.get("joint")!="hinge":
                    node["joint"]="hinge"
                    node["jointData"]= jd
            elif jt=="slider":
                if "joint" not in node or node["joint"]=="slider":
                    vol= compute_node_objs_volume_slider(node, base_path)
                    old_ms = node.get("motion_score",-1)
                    old_vol= node.get("slider_volume",-1)
                    do_override=False
                    if ms>old_ms:
                        do_override=True
                    elif abs(ms-old_ms)<1e-9:
                        if vol>old_vol:
                            do_override=True
                        elif abs(vol-old_vol)<1e-9:
                            do_override=True
                    if do_override:
                        node["joint"]="slider"
                        node["jointData"]= jd
                        node["motion_score"]= ms
                        node["slider_volume"]= vol
            else:
                # 其它类型(示例)
                old_ms = node.get("motion_score",-1)
                if "joint" not in node or old_ms< ms:
                    node["joint"]= jt
                    node["jointData"]= jd
                    node["motion_score"]= ms

    for c in node.get("children",[]):
        enrich_node(c, mobility_dict, base_path)
    node.pop("motion_score",None)
    node.pop("slider_volume",None)


# --------------------- 以下保留原先的 hinge/slider 箭头生成逻辑 ---------------------
...
# def generate_arrow_for_slider_hinge(joint_data,node):
#     """ (示例) 不实现 """
#     return None


def traverse_and_generate_arrows(node, base_path, generated_arrow=False):
    """
    DFS遍历 node，如果碰到 joint=>jointData，就只生成original类型的箭头
    并返回 1个数组(包含箭头mesh)
    """
    original_arrows=[]

    def gen_for_joint(jt,jd,nd):
        if jt=="hinge":
            part_objs= nd.get("objs",[])
            pm= load_and_get_part_meshes_hinge(part_objs, base_path)
            if pm:
                orig= generate_arrow_for_hinge_hinge(jd, pm, nd, base_path, arrow_style=None)
                return orig
            else:
                return None
        elif jt=="slider":
            orig = generate_arrow_for_slider_slider(jd, nd, base_path, arrow_style=None)
            return orig
        else:
            return None

    # 如果本节点有joint，且还没生成过箭头，就尝试生成
    if (not generated_arrow) and ("joint" in node) and ("jointData" in node) and ("objs" in node):
        jt= node["joint"]
        jd= node["jointData"]
        if "axis" in jd and "limit" in jd:
            arrow = gen_for_joint(jt,jd,node)
            if arrow: 
                original_arrows.append(arrow)
                generated_arrow=True

    # 递归子节点
    for c in node.get("children",[]):
        co = traverse_and_generate_arrows(c, base_path, generated_arrow)
        original_arrows.extend(co)
        if co:
            generated_arrow=True

    return original_arrows


def process_one_object(base_dir, yy_object_dir, fail_logger):
    """
    读取 mobility_v2.json + result_xxx => 为每个非noLimit关节生成箭头
    最终生成:
       yy_arrow/yy_arrow_{id}.obj 为每个关节ID生成单独的箭头
    (这个文件开头只写 arrow_{id} 无 base_geom)
    
    额外功能:
       yy_visualization/moveable_{id}.ply 为每个关节ID生成可视化
       yy_visualization/labels_{id}.npy 为每个关节ID生成标签
    """
    print(f"\n=== 处理对象目录: {base_dir} ===")

    file_candidates = ["result_after_merging.json","result_original.json","result.json"]
    mobility_data_path= os.path.join(base_dir,"mobility_v2.json")
    merged_json_path= os.path.join(yy_object_dir,"yy_merge_json.json")
    arrow_folder= os.path.join(yy_object_dir,"yy_arrow")
    os.makedirs(arrow_folder, exist_ok=True)

    if not os.path.exists(mobility_data_path):
        rs= f"[process_one_object] no mobility_v2.json: {mobility_data_path}"
        print(rs)
        fail_logger.append({"object_dir":base_dir,"reason":rs})
        return
    try:
        with open(mobility_data_path,"r",encoding='utf-8') as f:
            mobility_data= json.load(f)
    except Exception as e:
        rs= f"读取 mobility_v2.json 出错: {e}"
        print(rs)
        fail_logger.append({"object_dir":base_dir,"reason":rs})
        return
    ...
    # 创建以ID为键的mobility字典
    mobility_dict_by_id = {}
    mobility_dict_by_name = {}
    
    for item in mobility_data:
        if "id" in item and "name" in item:
            mobility_dict_by_id[item["id"]] = item
            # 同时保持原来的按name索引的字典，用于兼容现有代码
            if "name" in item:
                mobility_dict_by_name[item["name"]] = item

    def try_process_json(jname):
        jpath= os.path.join(base_dir,jname)
        if not os.path.exists(jpath):
            print(f"[{base_dir}] 不存在: {jpath}")
        ...
        # enrich => BFS/DFS赋予可能的joint
        for top_node in data:
            enrich_node(top_node, mobility_dict_by_name, base_dir)

        # 写出 yy_merge_json(保留所有关节的层次结构)
        try:
            with open(merged_json_path,"w",encoding='utf-8') as wf:
                json.dump(data, wf, indent=4)
            print(f"[{base_dir}] => {merged_json_path}")
        except Exception as e:
            return (False, f"写 yy_merge_json 出错:{e}")

        # 再读 => 生成箭头
        try:
            with open(merged_json_path,"r",encoding='utf-8') as rf:
                merged_data= json.load(rf)
        except Exception as e:
            return (False, f"读回yy_merge_json失败: {e}")
        if not merged_data:
            return (False,"合并json空 => 无法生成箭头")

        # 为每个关节生成箭头
        generated_arrows = {}  # 用于存储每个关节ID对应的箭头
        
        # 遍历mobility数据中的每个关节
        for joint_item in mobility_data:
            joint_id = joint_item.get("id")
            joint_type = joint_item.get("joint")
            joint_data = joint_item.get("jointData", {})
            
            # 跳过noLimit的关节
            if joint_data.get("limit", {}).get("noLimit", True):
                continue
                
            # 跳过非hinge和非slider的关节
            if joint_type not in ["hinge", "slider"]:
                continue
                
            # 找到对应的节点
            joint_name = joint_item.get("name")
            ...
            for top_node in merged_data:
                ...
                matching_node = find_node_by_name(top_node, joint_name)
                if matching_node:
                    break
            
            if not matching_node:
                print(f"[process_one_object] 未找到关节 {joint_name} (ID: {joint_id}) 对应的节点")
                continue
            ...
            if arrow:
                ...
                print(f"[process_one_object] 成功为关节 {joint_name} (ID: {joint_id}) 生成箭头")
            else:
                print(f"[process_one_object] 无法为关节 {joint_name} (ID: {joint_id}) 生成箭头")
        ...
        for joint_id, arrow_mesh in generated_arrows.items():
            ...
            try:
                ...
                print(f"[process_one_object] 导出箭头 => {arrow_obj_path}")
            except Exception as e:
                print(f"[process_one_object] 导出箭头 {joint_id} 失败: {e}")
        
        # 箭头生成和导出完成后，创建可视化（作为单独的步骤）
        if generated_arrows:
            try:
                print(f"[process_one_object] 箭头生成完成，开始创建可视化...")
                for joint_id in generated_arrows.keys():
                    if joint_id in mobility_dict_by_id:
                        joint_item = mobility_dict_by_id[joint_id]
                        joint_name = joint_item.get("name")
                        ...
                        if matching_node:
                            try:
                                ...
                                create_moveable_part_visualization(
                                    ...
                                    yy_object_dir=yy_object_dir
                                )
                            except Exception as e:
                                print(f"[process_one_object] 创建关节 {joint_id} 的可视化出错: {e}")
            except Exception as e:
                print(f"[process_one_object] 创建可视化过程出错: {e}")
                # 可视化失败不影响箭头生成的结果
            
            return (True, f"成功生成 {len(generated_arrows)} 个箭头")
        else:
            return (False, "未生成任何箭头")

    success=False
    for candidate in file_candidates:
        print(f"\n[{base_dir}] 尝试处理 {candidate}")
        ok, detail= try_process_json(candidate)
        if ok:
            success=True
            print(f"[process_one_object] {candidate} 成功 => 生成关节+箭头")
            break
        else:
            print(f"[process_one_object] 在 {candidate} 失败: {detail}, 下一个...")

    if not success:
        msg= f"{base_dir} => 3个 JSON 都无法生成关节&箭头"
        print(msg)
        fail_logger.append({"object_dir":base_dir,"reason":msg})
...
##############################################################################
#          3) 最终生成：yy_object => yy_merged.obj + 4*带箭头.obj(白+arrow)    #
##############################################################################
...
def create_moveable_part_visualization(base_dir, joint_id, joint_name, matching_node, yy_object_dir):
    """
    为指定关节ID创建可动部件可视化
    ...
    返回:
        生成的PLY文件路径
    """
    visualization_folder = os.path.join(yy_object_dir, "yy_visualization")
    ...
    # 同时保存标签为NPY文件，方便后续使用
    labels_path = os.path.join(visualization_folder, f"labels_{joint_id}.npy")
...
def process_one_object_from_urdf(base_dir, yy_object_dir, fail_logger):
    """
    从URDF文件和mobility_v2.json生成箭头
    最终生成:
       yy_arrow/yy_arrow_{id}.obj 为每个关节ID生成单独的箭头
    """
    ...
    if not os.path.exists(mobility_data_path):
        rs = f"[process_one_object_from_urdf] no mobility_v2.json: {mobility_data_path}"
        print(rs)
        fail_logger.append({"object_dir": base_dir, "reason": rs})
        return False
    ...
    # 读取mobility_v2.json，但主要用于获取joint_id
    try:
        with open(mobility_data_path, "r", encoding='utf-8') as f:
            mobility_data = json.load(f)
    except Exception as e:
    ...
    # 合并层次结构中的节点
    merged_data = root_links  # 明确定义merged_data为root_links
    
    # 创建URDF关节与mobility数据的映射
    # 使用一个基于URDF关节类型和层次关系的方法，而不是依赖名称
    joint_mappings = []
    
    # 首先，从URDF中收集所有非固定关节（活动关节）
    active_urdf_joints = []
    for joint_name, joint_data in urdf_data["joints"].items():
    ...
    print(f"[process_one_object_from_urdf] 发现 {len(joint_mappings)} 个活动关节")
    ...
    for mapping in joint_mappings:
        ...
        urdf_joint_data = mapping["urdf_joint"]["data"] 
        urdf_joint_name = mapping["urdf_joint"]["name"]
        joint_type = mapping["urdf_joint"]["type"]
        ...
        # 添加轴信息
        axis_xyz = urdf_joint_data.get("axis", [0, 0, 0])
        ...
        if arrow:
            generated_arrows[joint_id] = arrow
            print(f"[process_one_object_from_urdf] 成功为关节 {urdf_joint_name} (ID: {joint_id}) 生成箭头")
        else:
            print(f"[process_one_object_from_urdf] 无法为关节 {urdf_joint_name} (ID: {joint_id}) 生成箭头")
    ...
    whole_mesh = None
    try:
        # 尝试从textured_objs目录中加载所有OBJ文件并合并
        obj_files = []
        textured_objs_dir = os.path.join(base_dir, "textured_objs")
        if os.path.exists(textured_objs_dir):
            for obj_file in os.listdir(textured_objs_dir):
        ...
        if obj_files:
            ...
            for obj_path in obj_files:
                try:
                    ...
                    print(f"[process_one_object_from_urdf] 加载 {obj_path} 出错: {e}")
            
            if meshes:
                whole_mesh = trimesh.util.concatenate(meshes)
                print(f"[process_one_object_from_urdf] 成功加载整体模型，包含 {len(whole_mesh.faces)} 个面片")
    except Exception as e:
        print(f"[process_one_object_from_urdf] 加载整体模型出错: {e}")
    
    # 箭头生成完毕后，创建可视化（这是一个单独的步骤，不影响箭头生成）
    if generated_arrows and whole_mesh is not None:
        print(f"[process_one_object_from_urdf] 成功生成 {len(generated_arrows)} 个箭头，开始创建可视化...")
        
        # 创建可视化目录
        visualization_dir = os.path.join(yy_object_dir, "yy_visualization")
            ...
            if matching_node:
                try:
                    ...
                    if moveable_objs:
                        ...
                        print(f"[process_one_object_from_urdf] 关节 {joint_id} 找到 {len(moveable_objs)} 个可动部件OBJ")
                            ...
                            if os.path.exists(mesh_path):
                                try:
                                    ...
                                    for part_idx, part_centroid in enumerate(part_centroids):
                                        ...
                                        # 如果距离小于阈值，认为匹配成功并标记为可动(1)
                                        min_distance = distances[closest_face_idx]
                                        if min_distance < 0.05:  # 距离阈值
                                            face_labels[closest_face_idx] = 1
                                except Exception as e:
                                    print(f"[process_one_object_from_urdf] 加载 {obj_name}.obj 出错: {e}")
                        ...
                        # 导出PLY文件
                        output_path = os.path.join(visualization_dir, f"moveable_{joint_id}.ply")
                        colored_mesh.export(output_path)
                        
                        # 同时保存标签为NPY文件，方便后续使用
                        labels_path = os.path.join(visualization_dir, f"labels_{joint_id}.npy")
                        np.save(labels_path, face_labels)
                        
                        print(f"[process_one_object_from_urdf] 保存可视化结果到 {output_path}")
                except Exception as e:
...
def final_merge_for_object(base_dir, fail_logger):
    """
    对单个对象：
      1) 从URDF和mobility_v2.json生成箭头 => yy_object/yy_arrow/yy_arrow_{id}.obj (只含箭头)
      2) 将 textured_objs/*.obj 合并 => yy_object/yy_merged.obj (非arrow => base_geom)
      3) 再与每个箭头OBJ合并 => yy_merged_{id}.obj
         (arrow部分依旧是 arrow_{id}，object部分是 base_geom，不会覆盖箭头)
      
    额外功能:
      4) 为每个关节ID创建标记和可视化 => yy_visualization/moveable_{id}.ply
         显示关节相关的可动部件(绿色)和固定部件(红色)
    """
    yy_object_dir= os.path.join(base_dir,"yy_object")
    os.makedirs(yy_object_dir, exist_ok=True)

    # 1) 为每个关节生成箭头 - 优先使用URDF
    urdf_success = False
    if os.path.exists(os.path.join(base_dir, "mobility.urdf")):
        urdf_success = process_one_object_from_urdf(base_dir, yy_object_dir, fail_logger)
    
    # 如果URDF处理失败，回退到JSON处理
    if not urdf_success:
        print(f"[final_merge] URDF处理失败，尝试使用JSON文件")
        process_one_object(base_dir, yy_object_dir, fail_logger)
    ...
    # 3) 与每个箭头合并 => yy_merged_{id}.obj
    arrow_dir= os.path.join(yy_object_dir,"yy_arrow")
    ...
    for arrow_file in arrow_files:
        ...
        if os.path.exists(arrow_path):
            out_obj = os.path.join(yy_object_dir, f"yy_merged_{joint_id}.obj")
    ...
    try:
        ...
        if os.path.isdir(arrow_dir):
            ...
            if arrow_files:
                print(f"[final_merge] 找到 {len(arrow_files)} 个箭头文件，尝试创建可视化...")
                ...
                # 检查mobility_v2.json
                mobility_data_path= os.path.join(base_dir,"mobility_v2.json")
                if os.path.exists(mobility_data_path):
                    try:
                        with open(mobility_data_path,"r",encoding='utf-8') as f:
                            mobility_data = json.load(f)
                        ...
                        for arrow_file in arrow_files:
                            ...
                            # 从mobility_v2.json获取节点名称
                            for item in mobility_data:
                                if item.get("id") == joint_id:
                                    joint_name = item.get("name")
                                    break
                            ...
                            if joint_name:
                                ...
                                if os.path.exists(os.path.join(base_dir, "mobility.urdf")):
                                    # 从URDF中获取节点信息
                                    urdf_data = parse_urdf_file(os.path.join(base_dir, "mobility.urdf"))
                                    if urdf_data:
                                        root_links = []
                                        for link_name in urdf_data["links"]:
                                            is_root = True
                                        ...
                                        for root_node in root_links:
                                            matching_node = find_node_by_name(root_node, joint_name)
                                            if matching_node:
                                                break
                                ...
                                if matching_node:
                                    try:
                                        ...
                                        print(f"[final_merge] 创建关节 {joint_id} 的可视化出错: {e}")
                    except Exception as e:
                        print(f"[final_merge] 读取mobility_v2.json出错: {e}")
    except Exception as e:
        print(f"[final_merge] 创建可视化步骤出错: {e}")
        # 可视化步骤失败不影响主流程
...
def main():
    ...
    try:
        with open(id_sorted_report, "w", encoding="utf-8") as f:
            f.write("Arrow Creation Report (Sorted by ID)\n")
            f.write("================================\n\n")
