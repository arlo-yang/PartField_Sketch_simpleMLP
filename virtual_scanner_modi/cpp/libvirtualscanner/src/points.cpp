...
bool Points::write_ply(const string & filename) const {
  ...
  // 如果有面片索引，添加face_id属性
  if (info_->has_property(PtsInfo::kFaceIndex)) {
    outfile << "property int face_id\n";
  }
  ...
  if (info_->has_property(PtsInfo::kFaceIndex)) {
    // 如果有面片索引，输出格式包含face_id
    const int* face_ids = reinterpret_cast<const int*>(ptr(PtsInfo::kFaceIndex));
    for (int i = 0; i < n; ++i) {
      sprintf(pstr + i * len,
          "%.6f %.6f %.6f %.6f %.6f %.6f %d\n",
...
bool Points::set_points(const vector<float>& pts, const vector<float>& normals,
  ...
  if (!face_indices.empty()) {
    ...
    // !!! The channel of face_indices has to be 1
    if (1 != c || 0 != r) return false;
